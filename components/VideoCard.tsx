...
function VideoCard({ video, interaction, onToggleLike }: Props) {
  ...
  // Reusable Action Bar component showing ONLY Avatar, Tym, Comment, Share
  const renderActionBar = (isMobile: boolean) => (
    <div 
      className={`flex flex-col items-center gap-5 z-20 
        ${isMobile 
          ? "absolute right-4 top-1/2 -translate-y-1/2 lg:hidden" 
          : "hidden lg:flex self-center"
        }`}
    >
      {/* Avatar button with '+' overlay */}
      <div className="relative flex flex-col items-center mb-1">
        ...
        {/* Overlapping red plus button */}
        <button className="absolute -bottom-1.5 w-5 h-5 rounded-full bg-[#fe2c55] hover:bg-[#e12248] text-white flex items-center justify-center border-2 border-black focus:outline-none transition-transform duration-100 active:scale-90 z-10 shadow-lg">
      ...
      <div className="flex flex-col items-center gap-1.5">
        <div className="w-[46px] h-[46px] rounded-full bg-[#161616]/75 border border-white/[0.06] backdrop-blur-md flex items-center justify-center text-white/90 shadow-lg">
  ...
  return (
    <div className="h-dvh w-full snap-start bg-[#080808] relative flex items-center justify-center lg:gap-8 overflow-hidden">
      {/* Container holding the video frame and mobile elements */}
      {/* 
        MATHEMATICALLY PERFECT 9:16 VIEWPORT FITTING FORMULA:
        - Width: min(100vw, 100vh * 9/16)
        - Height: min(100vh, 100vw * 16/9)
        This guarantees the card remains exactly 9:16 and scales DOWN to fit smaller screen width or height 
        proportionally, preventing ANY horizontal or vertical overflow.
      */}
      <div 
      ...
      >
        ...
        {/* Video frame: rounded corners everywhere for a premium, sleek floating look */}
        <div className="relative h-full w-full rounded-2xl overflow-hidden shadow-[0_12px_40px_rgba(0,0,0,0.6)] border border-white/[0.03] bg-black">
          ...
          {/* Thin Pink/Red Progress Indicator Bar: bottom-14 on mobile to clear BottomNav */}
          ...
          {/* RENDER ACTION BAR INSIDE FOR MOBILE ONLY */}
          {renderActionBar(true)}
        </div>
      </div>

      {/* RENDER ACTION BAR OUTSIDE FOR PC ONLY */}
